...
def parse_sochic_product(html: str, source_url: str) -> ImportedSoChicProduct:
    ...
    title = _first_non_empty(
        _json_value(product_json, "name"),
        _text(soup.select_one("h1.product_title")),
        _text(soup.select_one("h1")),
    )
    if not title:
        raise ProviderImportError("No se pudo detectar el titulo del producto So Chic.")

    description = _first_non_empty(
        _json_value(product_json, "description"),
        _text(soup.select_one("#tab-description")),
        _meta_content(soup, "meta[name='description']"),
    )

    sku_raw = _first_non_empty(
        _json_value(product_json, "sku"),
        _text(soup.select_one(".sku_wrapper .sku")),
    )
    sku = _parse_int(sku_raw)

    final_price, original_price = _extract_prices(soup, product_json)
    if final_price is None:
        raise ProviderImportError("No se pudo detectar el precio del producto So Chic.")

    discount_percent = _discount_percent(original_price, final_price)
    image_urls = _extract_image_urls(soup, product_json, source_url)
...
def _validate_sochic_url(url: str) -> None:
    parsed = urlparse(str(url or "").strip())
    if parsed.scheme not in {"http", "https"} or parsed.netloc.lower() not in SOCHIC_HOSTS:
        raise ProviderImportError("Solo se aceptan URLs de productos de sochic.com.ar.")
    if "/product/" not in parsed.path:
        raise ProviderImportError("La URL de So Chic debe apuntar a un producto.")
    ...
    for script in soup.find_all("script", type="application/ld+json"):
        ...
        for node in _walk_jsonld(parsed):
            ...
            if isinstance(types, str):
                types = [types]
            if isinstance(types, list) and "Product" in types:
                return node
    return {}
...
def _extract_prices(soup: BeautifulSoup, product_json: dict[str, Any]) -> tuple[Optional[int], Optional[int]]:
    ...
    if final_price is None:
        offer_prices = list(_iter_offer_prices(product_json.get("offers")))
        if offer_prices:
...
def _extract_image_urls(soup: BeautifulSoup, product_json: dict[str, Any], source_url: str) -> list[str]:
    candidates: list[str] = []
    for value in _as_list(product_json.get("image")):
    ...
    for table in soup.select("table.vartable, table[class*='vartable']"):
        ...
        for row in table.select("tbody tr"):
            ...
            for idx in color_indexes:
                if idx < len(cells):
                    value = _squash_ws(_text(cells[idx]))
                    if value:
                        colors.append(value)
            for cell in row.select("td[class*='pa-color'], td[class*='attribute_pa_color']"):
