"""Bulk import orchestration for HOLIC catalog."""
...
def create_bulk_run(db: Session, *, triggered_by: str | None) -> ProviderImportRun:
    ...
    if active:
        raise BulkImportConflictError("Ya hay una importación masiva de HOLIC en curso.")

    run = ProviderImportRun(
        provider=PROVIDER,
        status="running",
        phase="discovering",
        progress_detail="Explorando catálogo HOLIC…",
...
def run_holic_bulk_import(
    ...
    *,
    ...
    try:
        set_run_phase(db, run, phase="discovering", progress_detail="Explorando catálogo HOLIC…")
        on_progress = make_progress_callback(db, run_id)
        urls = discover_holic_product_urls(session, on_progress=on_progress)
        set_run_phase(
            db,
            run,
            phase="importing",
        ...
        for url in urls:
            ...
            try:
                ...
                logging.exception("holic bulk import failed for %s", url)
                _log_item(
                    db,
                    run,
                    source_url=url,
        ...
        logging.exception("holic bulk import run %s aborted", run_id)
